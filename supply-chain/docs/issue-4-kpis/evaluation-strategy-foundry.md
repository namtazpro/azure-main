---
title: Evaluation Strategy with Foundry Evaluations (Code-First)
...
## Evaluation strategy with Foundry Evaluations

This page describes a **code-first** evaluation strategy for the [scenario classification agent](4a-scenario-classification.md), built on **Microsoft Foundry Evaluations** and driven from Python (not the UI). It is written to slot into the **agent development lifecycle** so that every prompt or model change is measured against a fixed benchmark before it reaches production.

For how to build the benchmark itself, see [Building the evaluation dataset](dataset-guidance.md).
...
| Per-scenario precision and recall | Aggregation | Locate weak scenarios; track the poor **unknown** recall called out in [4.a](4a-scenario-classification.md) |
...
- A golden dataset in **JSONL** - see [Building the evaluation dataset](dataset-guidance.md).
...
- **Tracing / telemetry** - emit traces from the classifier to Application Insights so production predictions, latencies, and reclassifications are observable. This supports the nightly self-learning loop described in [4.a](4a-scenario-classification.md).
...
- [ ] Golden dataset in JSONL with `ground_truth` labels - see [dataset guidance](dataset-guidance.md).
