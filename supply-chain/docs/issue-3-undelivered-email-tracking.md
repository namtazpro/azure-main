...
description: Why bounced outbound emails from the LET AI bot cannot be mapped back to the originating request, the non-delivery report correlation gap in Microsoft Graph, and why this change request may be descoped pending Microsoft-side investigation.
...
This is a **change request** the team may **descope or put on hold** unless a correlation approach is found. Before parking it, EY wanted to check with Microsoft.
...
At some stages the bot must email an external party, using addresses supplied by the **OTM** (Oracle Transportation Management) team - typically loading-location or location-related addresses. Two things go wrong:

1. **Placeholder addresses.** When OTM has no real address, it stores a default placeholder (for example `CSSundelivered@...`) that is not a valid mailbox.
...
- A **conversation ID** and a **messages array** (incoming and outgoing mails) are maintained per thread and stored in the system (Cosmos DB).
...
For these reasons the team **blocked the feature technically** in their development environment.
...
During the call Vincent saw **more header information** in his own Outlook (message hops, original message headers) than the team sees on their side. This may be an **extended feature in Office** or a mailbox/tenant configuration difference. Vincent asked Sourav to **forward the test email** to his Microsoft.com address to investigate why the available information differs.

## Status and next steps

- This is **hard to unpack** without deep investigation and is a candidate to **descope or hold**.
- Vincent to investigate on the Microsoft side **why the header information differs**, and whether the NDR headers / original message headers can carry a usable correlation ID.
- Directions worth evaluating (from earlier discovery): **validate the recipient address via Graph before sending** and route invalids to a **dead-letter queue** with a defined business process; review the earlier support-ticket trace.
- Owner: EY web-app team (Shalini / Rohit).

## Related

- [Backlog overview](README.md#item-3---cr-undelivered-email-tracking).
