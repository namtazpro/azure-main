---
title: Unilever LET AI Solution - Technical Backlog
description: Overview of the six technical backlog items for the Unilever LET AI logistics email automation solution, with detailed documentation added per item over time.
...
This documentation tracks the improvement work on the **Unilever LET AI** logistics email automation solution. Inbound logistics emails are redirected from ~60-67 specialist mailboxes into the application, where an AI classification agent (Semantic Kernel + Azure OpenAI) identifies the scenario, extracts entities, calls Oracle Transportation Management (OTM) and SAP, selects and fills a response template, and returns it to a specialist for review. Emails it cannot handle are routed back to the specialist mailbox.

The backlog is a set of **six items** that EY carved out of a larger turnaround plan - specifically the asks where they need Microsoft's support. It was compiled from two Microsoft - Unilever / EY discovery sessions:
...
| # | Item | Raised by | Support requested | Documentation |
|---|------|-----------|-------------------|---------------|
| 1 | Handling non-automated email | Saurav / Edyta | Feasibility | [Item 1 detail](issue-1-non-automated-email.md) |
| 2 | Centralized Outlook re-direction rules | Saurav / Edyta | Feasibility | [Item 2 detail](issue-2-redirection-rules.md) |
| 3 | CR: undelivered email tracking | HAP | Technical feasibility review | [Item 3 detail](issue-3-undelivered-email-tracking.md) |
| 4 | Response correctness & scenario classification | EY | Architecture / methodology review | [Item 4 overview](issue-4-kpis/README.md) |
| 5 | Self-learning capabilities | EY | Architecture / methodology review | Detail to follow |
| 6 | Evals | EY | Architecture / methodology review | Detail to follow |

## Item summaries

### Item 1 - Handling non-automated email

When LET AI cannot automate an email it returns it to the specialist as a **forward** rather than the original message, so the "From" becomes LET AI and the original sender is lost. Every return looks identical, forcing specialists to dig through the thread to find and copy the requester's address. Combined with working two inboxes (Outlook plus the web app), mailboxes become unmanageable and the old carrier/warehouse folder segregation breaks down. Volume is tied to the classification problem (see Item 4): emails with a confidence score below 75%, not-yet-live scenarios, and unmatched business groups all get forwarded back. **EY's higher-priority email item.**

*Ideas discussed:* convert the forward into a **redirect** to preserve the original sender; make redirection **dynamic** (back to the originating specialist); a Microsoft Graph "reply-to" workaround EY already explored; replacing mail rules with Power Automate / Logic Apps, or shifting away from email toward tickets or Teams notifications.
...
The bot sends outbound emails when it needs more information from a requester, TSP, or warehouse. Target addresses come from the central OTM database and are sometimes invalid or retired, so emails bounce. The undelivered notifications are **generic and cannot be tied back** to the original request or thread, so failures cannot be acted on. Recipients are dynamic and predominantly B2B (many on non-Unilever domains), a constant data challenge. Previously raised with Microsoft support without resolution.

*Ideas discussed:* a self-managed technical acknowledgement (detect a missing expected response); validate the recipient address via Graph before sending and route invalids to a **dead-letter queue** with a defined business process; review the earlier support-ticket trace. Owner: EY's web-app team (Shalini / Rohit).
...
Agreed direction from the sessions: split into small, focused workstreams with named owners on each side (for example email items with Mark, evals with Vincent); Unilever / EY to confirm the Graph permissions policy; EY to share the Microsoft support-ticket trace (Item 3) and the Graph "reply-to" thread (Item 1).

## Glossary

| Term | Meaning |
|------|---------|
| LET AI | The Unilever logistics email automation solution documented here |
